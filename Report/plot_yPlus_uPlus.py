...
def plot(CASE,case_VW,case_H):
  ...
  plt.loglog(yPlus_sim,uPlus_sim,'-r',label='Simulated')
  expDataFile=expDataDir+'Case_'+caseChar+'_exp.dat'
  case_exp=np.loadtxt(expDataFile)
  plt.loglog((case_exp[:,0]*exp_Utau1[CASE-1])/KIN_VIS,case_exp[:,1]/exp_Utau2[CASE-1],'--k',label='Experimental')
  plt.xlabel('y+')  
  plt.ylabel('u+')
  plt.title('y+ vs u+ for Case '+caseChar)
  plt.legend() 
  plt.savefig('./yPlus_uPlus/Compare_yPlus_uPlus'+caseChar+'.eps')
  plt.show()
  return;  
