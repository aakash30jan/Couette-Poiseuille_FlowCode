#v1 with clean dataset(not extrapolated !)
#training data combination of sim(within exp range) and exp
...
case_training_data=np.zeros(((case_sim_fit.shape[0]+case_exp.shape[0]),2))
case_training_data[:,0]=np.unique(np.append(case_sim_fit[:,0],case_exp[:,0]))
case_training_data[:,1]=np.unique(np.append(case_sim_fit[:,1],case_exp[:,1]))
...
plt.plot(case_sim_fit[:,0],case_sim_fit[:,1],'-g',label='training data simulated')
